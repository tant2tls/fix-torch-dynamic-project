...
**Files:** `torch/_inductor/lowering.py` (+29 −3), `test/inductor/test_custom_lowering.py` (+170 −1)
...
Because `inv_scales` now holds either an inverse scale *or* a deferred output size, `scale_fn`
tests the same predicate that chose it. `isinstance(scale, sympy.Expr)` alone is not enough:
`sympy.Integer(64)` is a `sympy.Expr` but concrete, so it takes the folded path above and
must not then be read as an output size — doing so divides by an already-inverted scale and
indexes far outside the input instead of crashing. `test_upsample_nearestnd_concrete_sympy_output_size`
pins that case for both `int` and `sympy.Integer` sizes.
...
gap: a plain `ops.truediv` in place of `div_rn` measures 12.70 ms, so the cost is the dynamic
index math, not the rounding mode.
...
The ATen `upsample_nearest*` ops do not reach this lowering: their decompositions are
registered on `CompositeImplicitAutograd`, so they fire before Inductor sees the op. Zero
